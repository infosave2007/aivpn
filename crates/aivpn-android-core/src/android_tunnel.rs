...
pub async fn run_tunnel_android(
    ...
    *ACTIVE_REGIONAL_HINTS_JSON
    ...
    // VpnService.protect(int) hook for the shared socket factory: exempts the
    // socket from the VPN via JNI (moved verbatim from the former
    // create_protected_udp_socket). On failure the fd is closed by
    // create_udp_socket itself — the closure must NOT close it.
    let protect = |fd: RawFd| -> Result<()> {
        let mut guard = vm
    ...
    let raw_udp_fd = create_udp_socket(dest, &session, &protect)?;
