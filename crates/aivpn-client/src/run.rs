//! Всё, что делает клиент, без привязки к тому, какой бинарь его запустил.
//!
//! Тело жило в `main.rs` и переехало сюда без изменения поведения. Смысл
//! переезда — [`run`] принимает список транспортов, которые эта сборка умеет
//! открывать. Публичный бинарь передаёт пустой список, и тогда единственный
//! путь датаграмм — прямой UDP-сокет, как было всегда. Сборке с
//! дополнительным транспортом не нужны ни фича, ни опциональная зависимость в
//! этом крейте: она поставляет собственный бинарь, вызывающий [`run`].
...
/// Точка входа клиента.
///
/// `extra` — транспорты, которые эта сборка умеет открывать сверх встроенного
/// прямого UDP. Пустой вектор — публичная сборка — означает, что открыть можно
/// только прямой UDP-сокет.
///
/// `guard` — платформенный хук, удерживающий сокеты транспорта вне туннеля.
/// Для прямого UDP достаточно [`aivpn_common::transport::no_socket_guard`]:
/// тот сокет выводится из туннеля маршрутом, а не хуком.
///
/// Функция не возвращает управление в обычной жизни клиента: как и прежний
/// `main`, она завершает процесс через `std::process::exit` на всех путях
/// выхода.
pub async fn run(
    extra: Vec<std::sync::Arc<dyn aivpn_common::transport::TransportFactory>>,
    guard: aivpn_common::transport::SharedGuard,
) {
    // Собирается один раз на процесс: реестр неизменен после старта, а каждая
    // итерация переподключения получает тот же Arc.
    let registry = std::sync::Arc::new(transport_registry(extra));
    ...
    loop {
        ...
        match AivpnClient::new(config) {
            Ok(mut client) => {
                client.set_transport_registry(registry.clone(), guard.clone());
